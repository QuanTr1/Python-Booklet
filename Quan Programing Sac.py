...
for i in range(1,students + 1):
    ...
    for a in range(1,periods + 1):
        ...
        while attendance.upper() not in ["A", "P"]:
            ...
            attendance = input(f"Enter attendance for {name} (P/A) for period {a}: ")
        #append the attendance of the person name list in uppercase
...
#Set a varible for the text file
myFile = open("data.txt", "wt")
#loop through the students data to get their name and attendance
for key, value in students_data.items():
    #Write their name and their attendance in a text file
    myFile.write(f"{key}: ")
    myFile.write(f"{value}")
    myFile.write("\n")
print("Data written is a file.")
myFile.close()
